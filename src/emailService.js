...
class EmailService {
  ...
  async sendNewsDigest(processedNews, trendingTopics) {
    if (
      ...
      console.log(
        "💡 Add EMAIL_USER, EMAIL_APP_PASSWORD, and RECIPIENT_EMAIL to .env file"
      );
      return false;
    }
...
`;
    ...
    <div class="section">
        <div class="section-title">
            <span class="emoji">${categoryIcons[category] || "📌"}</span> ${
...
`;
        ...
        html += `        <div class="news-summary">${this.formatTextForHTML(
          data.summary
        )}</div>\n`;
...
`;
          data.items.slice(0, 3).forEach((item, index) => {
            html += `            <div class="source-item">
                <div class="source-title">${index + 1}. ${this.escapeHTML(
              item.title
            )}</div>
                <div class="source-name">📰 ${this.escapeHTML(
