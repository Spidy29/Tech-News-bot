...
Provide a brief summary of the most relevant developments.`
        ...
        try {
            const prompt = prompts[category] || `Summarize these ${category} news articles for backend developers:\n\n${articlesText}`;
...
Provide 4-5 key insights with practical implications for backend developers. Be specific about timelines and actionable steps.`;
    ...
    async generateTrendingAnalysis(trendingTopics) {
        ...
        const topicsText = trendingTopics.map(t => `${t.topic} (${t.mentions} mentions)`).join(', ');
...
Keep it concise and practical.`;
    ...
    async generatePersonalizedRecommendations(allNews) {
        ...
        const categories = Object.keys(allNews).filter(cat => allNews[cat].length > 0);
        
        const prompt = `Based on today's tech news digest with ${totalArticles} articles across ${categories.join(', ')}, provide 3-4 personalized learning recommendations for a ${config.developer.experienceLevel} backend developer.
...
Make it actionable and relevant to current industry trends.`;
    ...
    generateEmptyCategorySummary(category) {
        const fallbacks = {
            ...
            security: "No critical security alerts today. Continue following security best practices: keep dependencies updated, use HTTPS, validate inputs, and monitor your systems.",
            ...
            frameworks: "No major framework updates today. Good time to review your current tech stack and plan any necessary upgrades.",
            global: "Global tech news quiet today. Focus on your development work and stay connected with the tech community."
        };
